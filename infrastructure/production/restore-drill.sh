...
proof_archive_dir=/mnt/archive_hdd_serv/telegram-orders/payment-proofs
...
proof_archive="$(find "$proof_archive_dir" -maxdepth 1 -type f -name 'payment-proofs-*.tar.gz' -printf '%T@ %p\n' | sort -nr | head -n1 | cut -d' ' -f2-)"
test -n "$proof_archive" && test -s "$proof_archive" || { echo "ERROR: no payment proof backup found" >&2; exit 1; }
tar -tzf "$proof_archive" >/dev/null
